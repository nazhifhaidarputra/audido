...
pub fn add_to_queue(ctx: Arc<CoreContext>, paths: Vec<String>) {
...
pub fn remove_from_queue(ctx: Arc<CoreContext>, id: usize) {
    let handle = ctx.tokio_handle.clone();
    handle.spawn(async move {
        {
            let mut queue = ctx.queue.lock().expect("queue poisoned");
...
pub fn clear_queue(ctx: Arc<CoreContext>) {
    let handle = ctx.tokio_handle.clone();
    handle.spawn(async move {
...
pub fn play_index(ctx: Arc<CoreContext>, index: usize) {
    let handle = ctx.tokio_handle.clone();
    handle.spawn(async move {
...
pub fn next(ctx: Arc<CoreContext>) {
    let handle = ctx.tokio_handle.clone();
    handle.spawn(async move {
        let next_idx = {
